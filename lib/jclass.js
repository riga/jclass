...
(function(ns, nsKey) {
    ...
    // store the current 'ns.JClass' to use it later on when 'noConflict' is called
    var _JClass = ns[nsKey];

    // define default options
    // when e.g. 'init' is used in the following comments, I'm of course referencing the value of
    // the 'initKey' option (and so on)
    // PLEASE NOTE:
    //      when extending an existing class, please use identical values for the options 'initKey',
    //      'superKey', 'privatePattern' and 'privateKey' in order to keep the prototype/inheritence
    //      model consistent
    var defaultOpts = {
        ...
        // when set to false, this is equivalent to 'privatePattern = null' plus 'privateKey = null'
        enablePrivacy: true,
                ...
                var protoFn = (function(name, fn) {
                    return function() {
                        ...
                        if (!opts.privatePattern || !opts.privatePattern.test(name))
                            this[opts.superKey] = _super[name];
                        ...
                        var tmpMethods, tmpMethod, tmpValues, initialKeys;
                        ...
                        // call the actual function whose scope will contains the additional
                        // temporary objects
                        ...
                        if (opts.privatePattern) {
                            ...
                            if (opts.tracking) {
                                // check if new private values have been added or removed (this
    ...
    // 'noConflict' will set 'namespace.JClass' to its original value and return JClass
